...
$targetFolder = $upload_dir['basedir'] . DS . 'wp-checkout' . DS . 'ajaxupload';
...
if (!empty($_FILES)) {
    ...
    if (!in_array($fileParts['extension'], $dangerFileTypes)) {
		if (move_uploaded_file($tempFile, $targetFile)) {
			...
			echo __('File could not be moved from tmp', "wp-checkout");
		}
	} else {
		echo __('Invalid file type', "wp-checkout");
	}
} else {
	echo __('No file data was posted', "wp-checkout");
}
