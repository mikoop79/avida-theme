...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
	...
	$out[] = 'var so = new SWFObject("'. $base .'images/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. $base .'images/open-flash-chart.swf?width='. $width .'&height='. $height . '&data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<param name="wmode" value="transparent" />';
    $out[] = '<embed src="'. $base .'images/open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" wmode="transparent" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
