<div style="width:auto; margin:auto; font-family:Arial, Helvetica, sans-serif;">
    <h1 style="text-align:right;"><?php _e('Supplier Receipt', $this -> plugin_name); ?></h1>
    <p>
                                            ...
                                            <?php if ($styles = maybe_unserialize($item -> styles)) : ?>
                                                <?php foreach ($styles as $style_id => $option_id) : ?>
                                                    ...
                                                    <?php if ($style = $wpcoDb -> find(array('id' => $style_id), array('id', 'title'))) : ?>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <?php foreach ($option_ids as $option_id) : ?>
                                                                        ...
                                                                        <?php if ($option = $wpcoDb -> find($conditions = array('id' => $option_id), $fields = array('id', 'title', 'addprice', 'price', 'operator', 'symbol'), $order = array('id', "DESC"), $assign = true)) : ?>
                                                                            <?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y") ? ' (' . $option -> symbol . $wpcoHtml -> currency_price($option -> price) . ')' : ''; ?>
                                                        ...
                                                        <?php else : ?>
                                                            <?php $wpcoDb -> model = $Option -> model; ?>
                                                            <?php $option = $wpcoDb -> find($conditions = array('id' => $option_id), $fields = "*", $order = array('id', "DESC"), $assign = true, $atts = array('otheroptions' => $styles)); ?>
                                                            <tr>
                                                                <td><?php echo $style -> title; ?></td>
                                                                <td>
																	<?php echo $option -> title; ?>
                                                                    <?php $optionprice = $wpcoHtml -> currency_price($option -> price); ?>
