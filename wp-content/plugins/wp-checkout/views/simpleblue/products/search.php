<div class="shortproduct">
	<div class="img">
		<?php if ($this -> get_option('cropthumb') == "Y") : ?>
            <?php echo $wpcoHtml -> link($wpcoHtml -> timthumb_image($product -> image_url, $this -> get_option('loop_thumbw'), $this -> get_option('loop_thumbh'), $this -> get_option('loopthumbq'), "thumb"), get_permalink($product -> post_id), array('title' => apply_filters($this -> pre . '_product_title', apply_filters($this -> pre . '_product_title', $product -> title)))); ?>
		<?php else : ?>
			<?php echo $wpcoHtml -> link($wpcoHtml -> image($product -> image -> name), get_permalink($product -> post_id), array('title' => apply_filters($this -> pre . '_product_title', apply_filters($this -> pre . '_product_title', $product -> title)))); ?>
		<?php endif; ?>
	</div>
